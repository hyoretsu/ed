#include "../headers/linked_list.h"
#include <stdio.h>
#include <stdlib.h>

struct node {
    int value;
    Node* prev;
    Node* next;
};

Node* ll_create_node(int value) {
    Node* node = (Node*) malloc(sizeof(Node));
    if (!node) {
        return NULL;
    }

    node->value = value;
    node->prev = NULL;
    node->next = NULL;

    return node;
}

int ll_is_empty(const Node* lhead) {
    return lhead == NULL;
}

int ll_size(Node* lhead) {
    int counter = 0;

    while (lhead != NULL) {
        counter += 1;
        lhead = lhead->next;
    }

    return counter;
}

int ll_insert_first(Node** lhead, int value) {
    Node* newNode = ll_create_node(value);
    if (!newNode) {
        return 0;
    }

    // Insert at beginning of the list
    newNode->next = *lhead;
    (*lhead)->prev = newNode;
    *lhead = newNode;

    return 1;
}

int ll_insert_last(Node** lhead, int value) {
    if (ll_is_empty(*lhead)) {
        return ll_insert_first(lhead, value);
    }

    Node* newNode = ll_create_node(value);
    if (!newNode) {
        return 0;
    }

    // Cycle to the end of the list
    Node* temp = *lhead;
    while (temp->next != NULL) {
        temp = temp->next;
    }

    // Insert at the end of the list
    temp->next = newNode;
    newNode->prev = temp;

    return 1;
}

int ll_exists(Node* lhead, int value) {
    if (ll_is_empty(lhead)) {
        return 0;
    }

    while (lhead != NULL) {
        if (lhead->value == value) {
            return 1;
        }

        lhead = lhead->next;
    }

    return 0;
}

void ll_print(Node* lhead, char* message) {
    printf("%s ", message);

    while (lhead != NULL) {
        printf("%d ", lhead->value);
        lhead = lhead->next;
    }

    printf("\n");
}

void ll_clear(Node** lhead) {
    Node* next = NULL;
    Node* temp = *lhead;

    while (temp != NULL) {
        next = temp->next;
        free(temp);
        temp = next;
    }

    *lhead = NULL;
}

int ll_remove(Node** lhead, int value) {
    Node* temp = *lhead;
    int found = 0;

    // Cycle through the list
    while (temp != NULL) {
        // Stop if given value was found
        if (temp->value == value) {
            found = 1;
            break;
        }

        temp = temp->next;
    }

    if (!found) {
        return 0;
    }

    if (temp->prev == NULL) { // Value was in the first element
        // Shift the beginning of the list
        *lhead = temp->next;
    } else {
        // Forward previous node to the next one
        (temp->next)->prev = temp->prev;

        // Forward next node to the previous one
        (temp->prev)->next = temp->next;
    }

    free(temp);

    return 1;
}

int ll_insert_sorted(Node** lhead, int value) {
    if (ll_is_empty(*lhead)) {
        ll_insert_first(lhead, value);

        return 1;
    }

    Node* newNode = ll_create_node(value);
    if (!newNode) {
        return 0;
    }

    Node* last = NULL;
    Node* temp = *lhead;

    // Cycle through the list until you find a >= value
    while (temp != NULL && temp->value < value) {
        last = temp;
        temp = temp->next;
    }

    // If given value's already the smallest one
    if (last == NULL) {
        newNode->next = *lhead;
        *lhead = newNode;
        return 1;
    }

    // Otherwise, insert new node between "last" and "next"
    newNode->next = last->next;
    last->next = newNode;

    return 1;
}
